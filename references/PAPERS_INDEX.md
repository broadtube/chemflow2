...
- **坂口 順一（千代田化工建設）「ＣＯ2を排出しない合成ガスの製造方法」WO2012140994A1**
  （PCT/JP2012/057127）優先日 2011-04-12 / 公開 2012-10-18 / 32 pp.
- → `WO2012140994A1_Chiyoda_syngas.pdf`（**スキャン、テキスト層なし**）

**該当箇所: 【0067】〜【0068】＝実施例セクション、公報 19 頁（PDF 21 頁目）**

> 【0067】…また、**カーボン活性**とは、下記式４で表されるものであり、この
> **カーボン活性の値が１を超えると触媒上にカーボンが析出しやすくなる**。
> 【0068】［式４］ **カーボン活性＝Ｋ×（Ｐco）²／（Ｐco₂）**
> ここで、**Ｋは２ＣＯ＝Ｃ＋ＣＯ₂の反応の平衡定数**であり、Ｐxは成分xの分圧である。

- **K の中身まで明示**されており、これは **Boudouard 経路そのもの**。
  AIChE 2008 は式を書いていないので、**千代田の定義が確認できるのはこの特許だけ**。
- 分圧の単位は明記なし。Boudouard の K が無次元である以上 atm 基準と解釈するほかなく、
  本実装と整合する。
- **表 5（公報 20 頁 / PDF 22 頁目）: 実施例 1〜4 の「出口のカーボン活性」がいずれも 0.90。**

| 項目 | 実施例1 | 実施例2 | 実施例3 | 実施例4 |
|---|---|---|---|---|
| S/C モル比 | 4.0 | **0.85** | 4.0 | − |
| CO2/炭素 モル比 | 9.6 | 0.82 | 9.4 | 2.4 |
| H2/CO モル比 | 2.0 | 2.0 | 2.0 | 0.5 |
| メタン含有率 [vol%] | 10 | 10 | 10 | 2.8 |
| 入口/出口温度 [℃] | 500/550 | **500/850** | 500/550 | 500/850 |
| 出口圧力 [MPaG] | 0.15 | **2.00** | 0.15 | 1.3 |
| GHSV [1/h] | 5950 | 265 | 5540 | 209 |
| **出口カーボン活性** | **0.90** | **0.90** | **0.90** | **0.90** |

**実施例 2 が本プロジェクトの検討条件に最も近い**（850 ℃・2.0 MPaG・S/C 0.85 の低水蒸気）。
こちらの検討（900 ℃・1.04 MPaG・H2O/CH4 = 0.72）と同じ領域を a_C = 0.90 に収めている。

- ⚠️ 保存 PDF はスキャン画像でテキスト層なし。位置はページを画像化して目視特定した。
- ⚠️ **Google Patents は当該箇所を【0091】/ 全 92 段落と表示するが誤り。**
  本 WO 公報の明細書は **【0075】（符号の説明）で終わっている**。
  ファミリー他文献のテキストが混在しているものと思われる。
...
- I. Alstrup. "A new model explaining carbon filament growth on nickel, iron, and Ni–Cu
  ...
  〔粒子径補正 `ΔG_c,dev = 2.6 + 93/d(nm)` の一次候補。**🚫 本プロジェクトでは不採用**〕

  **不採用の理由（2026-08-02 決定）**
  1. **Ni の使用を想定していない。** この式は Ni（および Ni-Cu・Ni-Fe）専用の補正。
     本件が前提にする触媒は千代田 CT-CO2AR® と同じ**貴金属担持**で、1993 年論文 Table 3 の
     とおり **Ru・Ir はウィスカーを作らない**（"No whiskers"）。Ni 向けの粒子径補正を
     貴金属系に当てる根拠がない。
  2. **一次確認ができていない。** 出所は二次資料（*Appl. Catal. A* 2018、有料）で、
     当該式が上記 Alstrup 1988 に載っているかは未確認。Ni についてすら 1993 Fig. 4 との
     一致は部分的（Ni-a は合うが Ni-b は 2 倍以上ずれる）。

  代わりに **§0-1 の 1993 年 Fig. 4**（原著が手元にあり Ni/Ru/Rh/Ir/Pt/Pd を実測）を使う。
  式と数表は `carbon_activity.html` §5.3 に**経緯の記録として残してあるが、実装・設計判断には
  一切使っていない**。
...
**⚠️ 設計方針への含意（2026-08-02 訂正）**

CT-CO2AR の売りは **a_C > 1 の領域でも運転できる触媒耐性**にある。
**ただし同社の設計実務はそうではない。** §0-2 の特許 表 5 のとおり、
**実施例 1〜4 のすべてで出口カーボン活性を 0.90 に設計している**。
触媒耐性を余裕として持ちつつ、プロセス設計としては a_C < 1 を守っている。

→ **本プロジェクトが課す a_C ≤ 1 は千代田の設計実務と一致する。**
（以前ここには「a_C ≤ 1 は千代田触媒を前提にしない保守的な条件」と書いていたが、
特許の実施例を確認して訂正した。保守側であること自体は §5 のとおり変わらない
——黒鉛基準・Ni/貴金属のずれのぶんは依然として余裕がある。）
