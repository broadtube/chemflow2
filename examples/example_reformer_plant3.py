r"""サンプル: 改質器 A〜D の DryGas を後工程 plant3 に流す。
...
example_plant3.py は無改造で import して使う（feed= で新鮮供給を差し替えられる作りの
ため）。出力は examples/output/reformer/ に分けてあり、既存の検討と混ざらない。
...
過去の検討（example_co2_removal.py）で、**ループ内の CO2 は順方向 WGS
（CO + H2O → CO2 + H2）の生成物側阻害として CO を守っていた**ことが分かっている。
CO2 を抜くと WGS が進んで CO が失われ、酢酸メチルが減った。D は DryGas の CO2 が
1.5 mol% しかないので、同じ機構で不利になりうる。
...
def run_case(tag, *, solve_tol=1e-4, progress_every=200):
    ...
    to_excel(rpb.streams, os.path.join(OUT, f"p3_{tag}_reformer.xlsx"),
             sheet=f"p3_{tag}_reformer", basis=BASIS, components=PLANT_C)
    export_mermaid(rpb, os.path.join(OUT, f"p3_{tag}_reformer.html"),
                   title=f"Reformer {tag}", style="diamond")

    # --- 後工程（plant3）---
    t0 = time.time()
    problem, streams, v_tot = p3.solve_with_sv(
    ...
    Purge, MA = by["7. Purge"], by["9. MethylAcetate"]
    print(stream_table(problem.streams, basis=["mol", "mole_frac"]))
    to_excel(problem.streams, os.path.join(OUT, f"p3_{tag}_plant3.xlsx"),
             sheet=f"p3_{tag}_plant3", basis=BASIS, components=PLANT_C)
    export_mermaid(problem, os.path.join(OUT, f"p3_{tag}_plant3.html"),
                   title=f"plant3 (feed = reformer {tag})", style="diamond")
    print(f"  → plant3 完了 ({time.time() - t0:.0f}s)", flush=True)
...
def main():
    ap = argparse.ArgumentParser(description="改質器 A〜D → plant3")
