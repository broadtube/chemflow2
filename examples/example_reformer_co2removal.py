r"""サンプル: 新しい改質器前提での CO2 除去位置の比較 — 後工程 plant3。

example_co2_removal.py の**改質器部分を example_reformer.py（供給 113 NL/h の新前提）に
差し替えた版**。除去位置・η スイープ・苛性ソーダ吸収塔・出力形式はすべて同じ構造。
既存ファイルには触れず、出力は examples/output/reformer/ に分けてある。

    baseline  除去なし（改質器の DryGas をそのまま plant3 へ）
    (a) 前段  改質器の下流・plant3 へ渡す手前で抜く   T-100（example_reformer.build の中）
    (b) 循環  凝縮器出口ガスとパージ分岐の"間"で抜く  T-101（p3.build の中）

    (a)  Feed(113 NL/h) → G1 → 凝縮 → DryGas ─[T-100]→ PlantFeed → plant3（塔なし）
...
本スクリプトは第一段としてパージ率を 5% に固定して比べる（--purge で上書き可）。

────────────────────────────────────────────────────────────────────────
改質条件 A〜D（example_reformer.py。詳細はそちらの docstring）
────────────────────────────────────────────────────────────────────────
                        A         B         C         D
    圧力 [MPaG]        1.41      1.41     0.417       0.1
    DryGas [NL/h]    120.13    132.84    183.15    201.41
    DryGas CO2 [mol%] 25.29     14.87      3.49      1.49   ← 抜ける CO2 の量
    改質 C 効率 [%]   55.77     66.83     86.09     93.70

⚠ **C・D はそもそも抜く CO2 がほとんど無い**（3.5 / 1.5 mol%）。除去率 η を振っても
効果は限定的なはず。それでも総当たりで回すのは、その縮退を実測で確認するため。
A・B は 15〜25 mol% あるので、以前と同じ構図（ループで 20 倍に濃縮）が成立する。

────────────────────────────────────────────────────────────────────────
⚠️ 事前の予測
────────────────────────────────────────────────────────────────────────
過去の検討（example_co2_removal.py）では **CO2 除去は 28 runs すべてで酢酸メチルを
減らした**。ループ内の CO2 が順方向 WGS（CO + H2O → CO2 + H2）の生成物側阻害として
CO を守っていたため。

ただし**回収 CO2 を改質器へ戻す前提なら評価は反転した**（投入炭素基準で 75 → 86%）。
今回の新前提では改質器の CO2 供給が 40〜52 NL/h と旧前提より多いので、戻す構成の
価値はさらに大きい可能性がある。
...
■ 総当たり（改質条件 4 種 × 除去位置 2 種 × η 3 点 = 28 runs）

**--reform は 1 条件しか取らないので 4 回に分けて実行する。**1 回の比較の中で
改質条件を固定しないと「改質条件を変えた効果」と「除去位置の効果」が混ざるため。

    PYTHONPATH=.:../reaction_rate/src python3 -u examples/example_reformer_co2removal.py --reform A --cases baseline,a_upstream,b_recycle --eta 0.5,0.9,0.99 --solve-tol 1e-4 --stop-at-tol
    PYTHONPATH=.:../reaction_rate/src python3 -u examples/example_reformer_co2removal.py --reform B --cases baseline,a_upstream,b_recycle --eta 0.5,0.9,0.99 --solve-tol 1e-4 --stop-at-tol
    PYTHONPATH=.:../reaction_rate/src python3 -u examples/example_reformer_co2removal.py --reform C --cases baseline,a_upstream,b_recycle --eta 0.5,0.9,0.99 --solve-tol 1e-4 --stop-at-tol
    PYTHONPATH=.:../reaction_rate/src python3 -u examples/example_reformer_co2removal.py --reform D --cases baseline,a_upstream,b_recycle --eta 0.5,0.9,0.99 --solve-tol 1e-4 --stop-at-tol
...
■ Windows (cmd)
    ...
    python -u examples\example_reformer_co2removal.py --reform A --cases baseline,a_upstream,b_recycle --eta 0.5,0.9,0.99 --solve-tol 1e-4 --stop-at-tol

■ 所要時間（実測ベースの見込み）

新しい改質条件での plant3 は 1 ケース平均 2.5 分（--stop-at-tol + x_scale jac の効果）。
28 runs で **約 70 分**。plant4 版は 40 runs で約 3 時間。

**スレッド数の固定について（実測で検証済み・効果は限定的）:** least_squares の
trust region 計算は内部で行列積を使うので、BLAS のスレッド数が変わると浮動小数の
加算順序が変わり、悪条件な本問題では求解の経路が分岐する。ここまでは事実:

    C_03MPaG b_recycle η0.99（元の条件で再現）
      スレッド自動   681 s   外側反復の nfev 62/250/277
      スレッド1固定  659 s   外側反復の nfev 57/321/201   ← 経路は変わった

だが**総時間は 5% しか変わらなかった**。同じコード・同じ入力で Linux(WSL) 691 秒に
対し Windows Native で 2 時間コースになりかけた事例があるが、**スレッド数はその主因
ではない**（5% であって 10 倍ではない）。残る候補は scipy/numpy のバージョン差、
CPU のマイクロアーキテクチャ差（OpenBLAS のカーネル選択）、reaction_rate の差。

固定しても害は無いので再現性のために付けてよいが、**遅さの対策としては
--stop-at-tol と --x-scale jac（既定）のほうが桁違いに効く**（691 s → 204 s）。
...
■ 手早く 1 条件だけ（baseline + (a) + (b) を η=0.9 で = 3 runs）

    PYTHONPATH=.:../reaction_rate/src python3 -u examples/example_reformer_co2removal.py --eta 0.9 --solve-tol 1e-4 --stop-at-tol

■ 引数

    --reform     A / B / C / D（既定 A）。example_reformer.CASES のキー。
...
■ 出力（examples/output/reformer/）

    co2removal_R{改質条件}_{ケース}_eta{η}_reformer.xlsx / .html
    co2removal_R{改質条件}_{ケース}_eta{η}_plant3.xlsx   / .html
    co2removal_R{改質条件}_{ケース}_eta{η}_caustic.xlsx  / .html
    co2removal_R{改質条件}_{ケース}_eta{η}_merged.xlsx   ← merge_xlsx.py が作る

ラベルに "R" を付けて旧検討（example_co2_removal.py）の出力と区別している。
出力先も examples/output/reformer/ に分けてある。
...
OUT = rf.OUT      # examples/output/reformer/（既存の検討と混ざらないよう分離）

#: 比較の土台にする改質条件。example_reformer.CASES の A〜D。
#: 1 回の比較の中では固定すること。動かすと「改質条件を変えた効果」と
#: 「除去位置の効果」が混ざって読めなくなる。
#:
#: 新しい改質条件では C・D の DryGas CO2 が 3.5 / 1.5 mol% しかない（A は 25.3、
#: B は 14.9）。**C・D はそもそも抜く CO2 がほとんど無い**ので、除去率 η を振っても
#: 効果は限定的なはず。それでも総当たりで回すのは、その縮退を確認するため。
DEFAULT_REFORM = "A"
...
def run_case(tag: str, eta: float, purge: float, reform: dict, label: str,
    ...
    rp, rst = rf.solve_case(reform, co2_removal=ref_removal)
    DryGas, PlantFeed = rst[3], rst[5]
    rsum = rf.summary(reform, rp, rst)
    print(f"改質器 {reform}: {rsum['温度 [℃]']:.0f}℃ {rsum['圧力']}  "
          f"CH4転化 {rsum['CH4 転化率 [%]']:.2f}% / CO2転化 {rsum['CO2 転化率 [%]']:.2f}% "
          f"/ DryGas {rsum['DryGas 合計 [NL/h]']:.1f} NL/h "
          f"(CO2 {rsum['DryGas CO2 [mol%]']:.2f} mol%)")
    ...
    ref_vent = next((s for s in rp.streams if s.name == "7. CO2Vent"), None)
...
def main():
    ...
    ap.add_argument("--reform", default=DEFAULT_REFORM,
                    help=f"土台にする改質条件（{'/'.join(rf.CASES)}）")
    ...
    if args.reform not in rf.CASES:
        ap.error(f"--reform は {list(rf.CASES)} のいずれか")
    reform = args.reform
    ...
    for tag in tags:
        if tag == "baseline":
            plan.append((tag, 0.0, f"R{args.reform}_baseline"))
        else:
            for eta in etas:
                plan.append((tag, eta, f"R{args.reform}_{tag}_eta{eta:g}"))
    ...
    if len(results) > 1:
        ...
        heads = {t: t[len(args.reform) + 2:] or "baseline" for t in results}
        print(f"{'指標':>24s}" + "".join(f"{heads[t]:>18s}" for t in results))
        for m in metrics:
            print(f"{m:>24s}" + "".join(f"{results[t][m]:18.4f}" for t in results))
        print("\n⚠ **投入炭素基準 収率**で比較すること（新鮮 CO 基準は改質器に入れた"
              "CH4・CO2 を分母に含めない）。")
        print("読み方: 「CO2 正味生成」が baseline より増えていれば、CO2 を抜いたぶん\n"
